...
async function askAI(userText) {
  ...
  const selectedModel = model.value.trim() || state.settings.model || 'gpt-5-mini';

  if (!base || !key) {
    return `Şu anda yapay zekâ bağlantısı yapılandırılmamış. “⚙ Ayarlar” bölümünden bir OpenAI-uyumlu API uç noktası ve anahtar ekleyebilirsin. Bu ekranın yerel demo özelliği acil durum güvenlik kontrolünü yine de çalıştırır.\n\n${safetyNote()}`;
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(`AI API ${response.status}: ${txt.slice(0, 300)}`);
  }
...
model.value = state.settings.model || 'gpt-5-mini';
$('#date').valueAsDate = new Date();
renderTracker();
